...
5. **Possible duplicate test factory.** The branch adds two files at `core/src/test/kotlin/cranium/core/service/util/factory/EnrichmentFactory.kt` AND `core/src/test/kotlin/cranium/core/service/util/factory/enrichment/EnrichmentFactory.kt`. One is likely a misplaced rename; deciding which to keep is a small cleanup but should happen before more tests reference whichever survives.
...
**Affected vault notes:** System Patterns (new pattern: event-dcranium WebSocket notifications), Domain documentation (new websocket domain), Infrastructure (WebSocket broker topology)
...
**Affected vault notes:** Domain Boundaries/Dependency Map, System Patterns (event-dcranium Temporal pipeline, pg_trgm similarity matching, canonical UUID ordering)
