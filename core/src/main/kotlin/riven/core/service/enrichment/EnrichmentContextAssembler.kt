 ...
 *
 * Self-loads the primary entity and entity type, delegates sentiment resolution to
 * [SentimentResolutionService], and returns [EntityKnowledgeView] — a fully structured
 * 8-section knowledge artifact over the entity.
 *
 * Plan 02-03: [assembleLegacyContext] bridge deleted alongside [EnrichmentContext]. The assembler
 * now has one public method: [assemble]. [relationshipTargetRuleRepository] dependency removed
 * (was only needed for the legacy bridge's topCategories computation).
 *
 * Dependency ceiling: ≤ 12 constructor parameters (ENRICH-02 / Decision 2A).
 * Current count: 11 (dropped relationshipTargetRuleRepository in Plan 02-03).
 *
...
class EnrichmentContextAssembler(
    ...
    // Constructor dep count: 11 (dropped relationshipTargetRuleRepository; ceiling ≤ 12).
    ...
    // ------ Private helper: relationship definitions map ------
